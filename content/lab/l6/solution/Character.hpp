...
public:
	...
	void addMana(int amount) { mana = std::min(mana + amount, maxMana); std::cout << "Mana increased by " << amount << ". Current mana: " << mana << std::endl; }
...
public:
	BattleMage(const std::string& name, int health = 100, int mana = 100, int meleeDmg = 10, int spellDmg = 15)
		: Character(name, health), CanCastSpells(mana), CanUseMelee(),
		baseMeleeDamage(meleeDmg), baseSpellDamage(spellDmg) {
	}
