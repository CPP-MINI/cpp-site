...
void BattleMage::castSpell(Character* target) {
	...
	std::cout << name << " casts a BattleMage spell on " << target->getName() << " for " << damageDealt << " magic damage!" << std::endl;
	target->takeDamage(damageDealt);
}
...
void BattleMage::performMeleeAttack(Character* target) {
	...
	std::cout << name << " strikes " << target->getName() << " with a BattleMage attack for " << damageDealt << " physical damage!" << std::endl;
	target->takeDamage(damageDealt);
}
...
void Rogue::backstab(Character* target) {
	...
	if (typeid(*target) != typeid(Mage) && typeid(*target) != typeid(BattleMage) && dynamic_cast<CanCastSpells*>(target) == nullptr) {
		int damageDealt = backstabDamage;
		std::cout << name << " successfully backstabs " << target->getName() << " for " << damageDealt << " massive damage!" << std::endl;
		target->takeDamage(damageDealt);
	}
	else {
