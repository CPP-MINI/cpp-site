...
**Uwaga o typie `size_t`**: W sygnaturach tych funkcji używany jest typ `size_t`, który służy do reprezentowania rozmiarów obiektów w pamięci. W przeciwieństwie do `int` (który może być ujemny), `size_t` jest typem **bez znaku specjalnie zaprojektowanym** do reprezentowania maksymalnego rozmiaru dowolnego obiektu w pamięci. Różni się również od `unsigned int`, ponieważ automatycznie dostosowuje się do możliwości adresowania platformy (np. 32 lub 64 bity), co gwarantuje możliwość przechowania rozmiaru każdego obiektu.
