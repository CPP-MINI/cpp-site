...
char l4::base32::Encoder::encodePlace(int bit_idx, std::byte current_byte, std::byte next_byte)
{
    // Move 5 bits ahead
...
void l4::base32::Encoder::appendBytestream(const std::byte *bytestream, std::size_t size)
{
    ...
    while (bit_idx < 8) // Tutaj potencjal na if zamiast while - czasami brak pojedynczego znaku na koniec
    {
        _buff.push_back(encodePlace(bit_idx, current_byte, next_byte));
        bit_idx = bit_idx + 5; // Tutaj tez nieskonczona petla jesli bedzie to modulo 8
    }

    // Insert as much padding as it is required
    int padding_count = k_bit_idx_to_padding_count[bit_idx % 8]; // Tu fajny potencjal na brak module 8. Wynik - nieskonczona petla)
